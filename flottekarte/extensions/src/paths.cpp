...
xy_t flottekarte::boundary_intersection(
    const xy_t& out,
    const xy_t& in,
    double xmin,
    double xmax,
...
static xy_t spherical_stereographic(
    const geo_t& p,
    const geo_t& center,
    double a
)
...
refine_and_split_discontinuity(
    const std::vector<std::pair<geo_t,xy_t>>& path,
